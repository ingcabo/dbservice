...
cargo:analista
    ...
    */
			...
			$exist       = $this->Model_usr->count_by(array("username"=>$usr));
			//consulto el nombre de usuario	y el email	
			$exist_email =	$this->Model_usr->count_by(array("email"=>$email));	

			//validar si existe almenos un nombre de usuario a consultado	
			if (($exist >= 1) || ($exist_email >= 1)){

				// aca respondo que el reistro ya existe
				$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'),  $this->config->item('rest_message_field_name') => "email/nombre de usuario existe", $this->config->item('data') =>'false'),REST_Controller::HTTP_BAD_REQUEST);
				}else{
				...
				if (!$usr_id){
					...
					$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') , $this->config->item('rest_message_field_name') => 'un error al crear el usuario', $this->config->item('data') =>'false'),REST_Controller::HTTP_BAD_REQUEST);
					...
					$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name')  => "Usuario Creado", $this->config->item('data') => $this->config->item('dataT')),200);
		        ...
		        if ($usr_datos['level'] == 3){
					...
					$usr = $this->Model_usr->get_by(array("id"=> $id, "status"=> "active" ));
				    ...
				    $safe_email = !isset($data['email']) || $data['email'] == $usr['email'] || !$this->Model_usr->get_by(array('email'=> $data['email']));		

				    if (!$safe_email){
				    	//respuesta email no seguro
				    	$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => "email ya existe", $this->config->item('data') => $this->config->item('dataF')),400);
				    	...
				    	$updated= $this->Model_usr->update($id,$data);
									...
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => 'un error al actualizar el usuario', $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
														
									}else{
									// si existe update muestro mensaje						
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') => 'Registro Actualizado', $this->config->item('data') => $this->config->item('dataT') ),200);
									}
											
						}else{
								// muestro registro existente
								$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => 'usuario no existe', $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
							...
							//aca muestro que faltan datos en el formulario o no cumplen con lo esperado
						$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'),  $this->config->item('rest_message_field_name') => $this->form_validation->get_errors_as_array(), $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
		...
		function usr_get(){
				...
				$criterio = array("id"=> $usr_datos['user_id'] , "status"=> "active" );
				}else{
				//es super usuario puede consultar el id de otro usuario o una consulta sin id que le regresaria todos los actuivos eso es criteriop	
							...
							if  ($this->uri->segment(3)){
								...
								$criterio = array("id"=> $usr_id , "status"=> "active" );	
							}else{
								$criterio = array("status"=> "active");
							}
				...
				if (isset($usr['id'])){
				  ...
				  $this->response(array($this->config->item('rest_status_field_name')=> $this->config->item('fallo'), $this->config->item('rest_message_field_name') => 'Consulta no exitosa', $this->config->item('data') => $this->config->item('dataF')),200);
				//$this->response($usr);
				
				}else{
				//si hay resultado
						if (count($usr) > 0){
						  $this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') =>"Consulta Satisfactoria", $this->config->item('data') =>$usr),200);
						//  $this->response($usr);
						}else{
						 $this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') =>"Consulta no Satisfactoria", $this->config->item('data') => $this->config->item('dataF')),400);
		...
		function usr_delete(){
				...
				$usr = $this->Model_usr->get_by(array("id"=> $usr_id, "status"=> "active" ));
				...
				$deleted = $this->Model_usr->update($usr_id,$data);
								...
								if (!$deleted){
									// si no hay delete
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'un error al aeliminar el usuario', $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
								
								}else{
								//exito
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') => "Usuario Eliminado", $this->config->item('data') =>$this->config->item('dataT')),200);				
								}
																
				}else{
					//faltan datos en el formulario
				$this->response(array($this->config->item('rest_status_field_name')=> $this->config->item('fallo'), $this->config->item('rest_message_field_name') => "el usuario no existe", $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
				}
}
...
function access_get(){
				...
				//es super usuario		
							...
							if  ($this->uri->segment(3)){
								$usr_id = $this->uri->segment(3);
								$criterio = array("user_id"=> $usr_id , "status"=> "active" );	
							}else{
								$criterio = array("status"=> "active");
							}
				  ...
				  $this->response(array($this->config->item('rest_status_field_name')=> $this->config->item('fallo'), $this->config->item('rest_message_field_name') => 'Consulta no exitosa', $this->config->item('data') => $this->config->item('dataF')),200);
					...
					// si si hay registros
						...
						//  $this->response($usr);
							...
							//si no hay registros
						 $this->response(array($this->config->item('rest_status_field_name') => 'success', $this->config->item('rest_message_field_name') =>"Consulta no Satisfactoria", $this->config->item('data') => $this->config->item('dataF')),200);
						// $this->response(array('status' => 'error', 'msg' => 'error_details'), 404);  
						 }
				}
		}
...
//lo parametros de la cabecera
		...
		// ya explicamos esta funcion en usr_put
			...
			//consulto 				
			$exist = $this->model_usracc->count_by(array("controller"=>"/".$control,"function" => $function, "metodo" => $metodo, "status" => "active", "user_id"=>$usr_id));
			

			//validar si existe almenos un nombre de usuario a consultado	
			if (($exist >= 1) ){

				$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => "Registro duplicado",  $this->config->item('data') =>$this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
				}else{
				//si no existe lo ingreso 
			...
			//insertar permiso
		 	$id= $this->model_usracc->insert($data);
				...
				if (!$id){
					...
					$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'un error al crear el registro', $this->config->item('data') =>$this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
									
				}else{
											
					$this->response(array($this->config->item('rest_status_field_name')=> $this->config->item('bien'), $this->config->item('rest_message_field_name') => "Registro Creado",  $this->config->item('data') => $this->config->item('dataT')),404);
				}
		...
		function access_post(){
			 ...
			 // aca consultamos los permisos
			   $usracc = $this->model_usracc->get_by(array("id"=> $usracc_id, "status"=> "active" ));
			...
			if ($this->form_validation->run('access_post') != false){
    		 ...
    		 $safe_metodo =   $this->model_usracc->verifica_metodo_update_exist($data['controller'],$data['function'],$data['metodo'],$usracc_id,$data['user_id']);	
										...
										if ($safe_metodo > 0){
												...
												$this->response(array($this->config->item('rest_status_field_name') =>$this->config->item('fallo'), $this->config->item('rest_message_field_name') => "Acceso Duplicado para el usuario", $this->config->item('data') => $this->config->item('dataF')),404);
											}
										...
										if (!$updated){
												...
												$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'Un error al actualizar el acceso', $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
												
												}else{
												
												$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') => "Acceso Actualizado", $this->config->item('data') => $this->config->item('dataT')),200);
										}
																						
						}else{
											
												$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => $this->form_validation->get_errors_as_array(),$data=>$this->config->item('dataF')),404);
				...
				$this->response(array($this->config->item('rest_status_field_name')=> $this->config->item('fallo'), $this->config->item('rest_message_field_name')=> "Acceso no existe", $this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
				}
...
function access_delete(){
				...
				$id = $this->uri->segment(3);			
				$this->load->model('model_usracc');
				$metodo = $this->model_usracc->get_by(array("id"=> $id, "status"=> "active" ));
				...
				$deleted = $this->model_usracc->delete($id);
								...
								if (!$deleted){
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => 'un error al eliminar el access', $this->config->item('data')=> $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
								
								}else{
								
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') => "Ecceso eliminado", $this->config->item('data') => $this->config->item('dataT')),200);				
								}
			...
			$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => "el access no existe",$this->config->item('data') => $this->config->item('dataF')),REST_Controller::HTTP_BAD_REQUEST);
				}
}
...
*/
	...
	//		$nombre = $usr_data['username'];
			...
			$uk = $this->model_acc->get_by(array("username"=> $username, "password" => md5($password),"status"=> "active" ));
		...
		// si se genera la llave y se logra guardar 
		 	...
		 	//actualiso las llaves de access
 						$updated = $this->model_usracc->update_key($uk['id'], $generate);
					 				...
					 				if ($updated >= 1){
					 					...
					 					$response = array(array("key" => $generate, "id" => $uk['id'],'keys_expire'=>$this->config->item('rest_time_keys_expire'),"level"=>$uk['level']));
					 					
					 				


									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'),$this->config->item('rest_message_field_name')=>"Bienvenido","key"=> $generate,"data" => $response),200);
									}else{
									$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name')=> "En estos momentos no  podemos asignarle permisos",$this->config->item('data')=> $this->config->item('dataF')),400);
									}
			                        //$this->response(array('status' => 'Faliured' ,"menssage" => "uno", "key"=> Null,400));
						}else{

								$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name')=>"En estos Momentos no podemos atender su inicio de sesion",$this->config->item('data') => $this->config->item('dataF')),400);
			...
			$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => "Usuario o Contraseña erroneos", $this->config->item('data') => $this->config->item('dataF')),400);
			...
			$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'), $this->config->item('rest_message_field_name') => "datos imcompletos", $this->config->item('data') => $this->config->item('dataF')),400);
	...
	/*esta funcion solo entrega la pregunta secreta a partir de un user_name valido esto es para iniciar el proceso de recuperacion de contraseña*/
			...
			foreach ($headers as $header => $value) {
			   ...
			   $usrdat = $this->Model_usr_p->get_by(array("username"=> $usr_data['username']));
			...
			if (count($usrdat) > 0){
				...
				// mostramos el resultado
					...
					$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'),$this->config->item('rest_message_field_name')=>"Muestra Pregunta Secreta","data" => $response),200);

			}else{
				//usuario no existe
		$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo'),$this->config->item('rest_message_field_name')=>"No existe el usuario","data" => $this->config->item('dataF')),500);
    ...
    /* esta funcion permite cambiar la contraseña a partir de responder correctamente la pregunta secreta ya entregada en la funcion get anterior*/
			...
			// si viene lo que esperamos en config/form_validation.php
			   ...
			   $usrdat = $this->Model_usr_p->get_by(array("pregunta1"=> $usr_data['pregunta1'],"respuesta1"=> $usr_data['respuesta1'],"username" => $usr_data['username']));

			   //asignamos un array con password para set de la nueva contraseña
			   $datos = array("password"=> $usr_data['password']); 
						   ...
						   if (count($usrdat) >= 1 ){
						   		// si es valida la respuesta hacemos update de contraseña
						   			$updated= $this->Model_usr_p->update($usrdat['id'],$datos);	
						   			 ...
						   			 $this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'Respuesta Erronea', $this->config->item('data') =>$this->config->item('dataF')),500);
						   }
						   ...
						   if ($updated){
						   	...
						   	// si si se actualizo la contraseña
						   		 $this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') => "Clave Actualizada", $this->config->item('data') => $this->config->item('dataT')),200);

						   }else{

						   		 $this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'No puede actualizar clave', $this->config->item('data') =>$this->config->item('dataF')),500);
			...
			function prerecu_put(){
			        ...
			        $datos  = array("password"=> ltrim(rtrim(($usr_data['password']))));
			        $updated= $this->Model_usr_p->update($id,$datos);


			        }else{

			        	$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'Escriba su nueva Contraseña', $this->config->item('data') =>$this->config->item('dataF')),500);
			        ...
			        if ($updated){
			        	...
			        	$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('fallo') ,$this->config->item('rest_message_field_name') => 'No se puede modificar el registro', $this->config->item('data') =>$this->config->item('dataF')),500);
    ...
    function postilion_get(){
    		...
    		$this->response(array($this->config->item('rest_status_field_name') => $this->config->item('bien'), $this->config->item('rest_message_field_name') => "postilion", $this->config->item('data') => $this->config->item('rest_time_keys_expire')),200);
