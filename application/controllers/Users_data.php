...
//username password nombres apellidos pregunta1  respuesta1 email cargo
			...
			if (($exist >= 1) || ($exist_email >= 1)){
				...
				$this->response(array("staus" =>"Error", "message" => "email duplicado","user"=>$usr,"studenarray"=>$data),REST_Controller::HTTP_BAD_REQUEST);
				}else{
				...
				if (!$usr_id){
					...
					$this->response(array('status' => 'Faliured' ,'message' => 'un error al crear el usuario','nombre'=>$usr,REST_Controller::HTTP_BAD_REQUEST));
									
				}else{
											
					$this->response(array("staus" =>"Susses", "message" => "user created","studenarray"=>$data,"id_usuario"=>$usr_id));
				}
				    ...
				    if (!$safe_email){
				    	...
				    	$this->response(array("staus" =>"Error", "message" => "email ya existe","email rechazado"=>"email rechazado","studenarray"=>$data));
						...
						$this->response(array('status' => 'Faliured' ,'message' => 'un error al actualizar el usuario',REST_Controller::HTTP_BAD_REQUEST));
											
						}else{
												
						$this->response(array("staus" =>"Susses", "message" => "usuario actualizado","data"=>$data ,200));
						}
						...
						$this->response(array("staus"=>"Faliured", "message"=> "el usuario no existe",REST_Controller::HTTP_BAD_REQUEST));
