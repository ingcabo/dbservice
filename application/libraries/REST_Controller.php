...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
      ...
      //  $this->mbo->dbd = $this->load->database('my_api', TRUE);


        // Load DB if its enabled IMPORTANTE
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if ($this->config->item('force_https') && $this->request->ssl === FALSE)
        {
            $this->response([
                    ...
                    $this->config->item('data') =>  $this->lang->line('dataF')
                ], self::HTTP_FORBIDDEN);
        }
        ...
        $use_key = !(isset($this->methods[$controller_method]['key']) && $this->methods[$controller_method]['key'] === FALSE);
          ...
          if (strtolower($this->router->class) == 'myrest' ){
         ...
         if (strtolower($this->router->class) == 'users' && (strtolower($this->router->method) =='usr' || strtolower($this->router->method) =='session') && strtolower($this->request->method) =='put'){
            ...
            $this->_allow = true;
             ...
             /*
                  ...
                  'alow' =>$this->_allow,
                  'use' => $use_key,
                   "aca llego"=>  $this->config->item('rest_enable_keys')
                ], self::HTTP_NOT_ACCEPTABLE);
            */
                ...
                if (strtolower($this->router->class) == 'mquery' && (strtolower($this->router->method) =='compra' || strtolower($this->router->method) =='detallecompra')  && (strtolower($this->request->method) =='get')){
                  ...
                  if (strtolower($this->router->class) == 'users' && strtolower($this->router->method) =='prerecu' && (strtolower($this->request->method) =='get'|| strtolower($this->request->method) =='post') ){
        ...
        if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === FALSE)
        {
            ...
            $this->response([
                   'VALOR REST KEY' => $this->rest->key,
                  //  'VALOR USE KEY' => $use_key,
                    //'que demonios' => $this->config->item('rest_enable_keys_false'),
                    $this->config->item('rest_status_field_name') => $this->config->item('fallo'),
       ...
       $congTiempo = $this->config->item('rest_time_keys_expire');
       $tiempokey = $this->key_expire();

     if($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE  &&  $congTiempo <  $tiempokey)
      {
        ...
        if ($this->config->item('rest_enable_logging') && $log_method)
            {
                $this->_log_request();
                 ...
                 $this->response([
                   // 'VALOR REST KEY' => $this->rest->key,
                   //  'VALOR USE KEY' => $use_key,
                   //  'ACCES'         =>$this->_check_access(),
                  //'tiempo_llave'     =>  $tiempokey,
                 //   'tiempo config'  =>  $congTiempo,

                    $this->config->item('rest_status_field_name') => $this->config->item('fallo'),
                    $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_api_key_time_limit_all'),
                    $this->config->item('data') =>  $this->lang->line('dataF')
                ], self::HTTP_NOT_ACCEPTABLE);

     }

        // Check to see if this key has access to the requested controller ACA DEBERIA COLOCAR MI MANTEQUILLA
if ($this->config->item('rest_enable_keys') && $use_key && empty($this->rest->key) === FALSE && $this->_check_access() === FALSE)
    ...
    protected function remove_header($value){

                unset($value['Accept']);
                unset($value['Accept-Encoding']);
                unset($value['Accept-Language']);
    ...
    protected function _log_request($authorized = FALSE)
    {
       ...
       $headers = $this->remove_header($headers);
    ...
    protected function _check_access()
    {
        ...
        return $this->rest->db
            ...
            ->where('function',$this->router->method)
            ->where('metodo',$this->request->method)
            ->get($this->config->item('rest_access_table'))
            ->num_rows() > 0;
    }

    /*ver si a llave esta activa*/
 /**
     * Check to see if the API key has access to the controller and methods
     *
     * @access public
